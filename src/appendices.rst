...
记住 :math:`L_j` 是 :math:`k` 右移 :math:`j` 位，所以 :math:`L_0` 是
:math:`k` 右移 0 位，就是 :math:`k` 本身。这意味着 :math:`g^k`，
我们试图计算的数字，与 :math:`g^{L_0}` 相同。通过从
:math:`g^{L_{t - 1}}`（:math:`g` 提升到 :math:`k` 最高位的幂）开始，
并迭代向下到 :math:`g^{L_0} = g^k`，我们有一种优雅的归纳方法来计算
:math:`g^k`，基于两个简单的递归规则。

这个算法的重要部分是恒定的操作数量。如果 :math:`k_j = 0`，计算
:math:`g^{L_j}` 涉及一次平方，:math:`g^{H_j}` 涉及一次乘法；如果
:math:`k_j = 1`，则相反。无论 :math:`k` 的任何位是什么，您每个位需要
一次平方操作和一次乘法操作。

用 Python 实现 Montgomery ladder
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

这个算法应用于模幂运算的 Python 实现出奇地简洁：
...
这段代码没有显示 ``bits`` 的定义：它产生其参数的二进制展开。
Python 默认不提供；``bin`` 接近，但它产生一个字符串：``bin(100)``
求值为 ``0b1100100``。``a, *b = bits(...)`` 构造将 ``bits(...)`` 的
第一项分配给 ``a``，所有剩余位分配给 ``b``，有效地只是跳过第一位。

这里要注意的重要事情是，无论指数的特定值是什么，每个位有一次平方、
一次乘法和一次模操作。请记住，这并不一定使整个算法采用恒定时间，
因为单独的平方和乘法操作不一定恒定时间。

离散对数
~~~~~~~~~~

就像减法是加法的逆，除法是乘法的逆，对数是幂的逆。
在常规算术中，:math:`b^x = y`，如果 :math:`x = \log_b y`。
这读作“:math:`b` 的 :math:`x` 次幂是 :math:`y`”和“:math:`y` 相对于
:math:`b` 的对数是 :math:`x`”。模算术中的等价物称为"离散对数"。

与除法一样，如果您从作为不同运算符的逆的定义开始，很容易想出例子。
例如，由于 :math:`3^6 \equiv 9 \pmod {15}`，我们可以定义
:math:`6 \equiv \log_3 9 \pmod {15}`。与模逆不同，计算离散对数通常很困难。
没有正式证明计算离散对数*本质上*复杂；我们只是没有找到任何有效的算法
来做到这一点。因为这个领域得到了广泛研究，我们仍然没有非常快速的通用算法，
我们认为基于计算离散对数是困难的假设来建立协议安全性是安全的。

有一个用于有效计算离散对数的理论算法。然而，它需要量子计算机，
与我们今天使用的经典计算机根本不同类型的计算机。虽然我们可以建造这样的
计算机，但我们只能建造非常小的。我们量子计算机的有限大小强烈限制了
我们可以解决的问题。到目前为止，它们更像是一个孩子可以在脑海中做的
算术类型，而不是从性能王座上驱逐顶级经典计算机。

计算离散对数的复杂性，加上计算其逆（模幂运算）的相对简单性，是许多
公钥密码系统的基础。常见示例包括 RSA 加密原语和 Diffie-Hellman 密钥
交换协议。

虽然基于离散对数问题的密码系统在适当的参数选择下被认为是安全的，
但将来 certainly 有改变的方式。例如：

- 数论的理论突破可能使离散对数比我们目前认为的更易于计算。
- 量子计算的技术突破可能导致足够大的量子计算机。
- 经典计算的技术突破以及性能和成本降低的持续渐进增加可能增加
  可以使用经典计算机处理的问题大小。

离散对数计算与数分解问题密切相关。它们仍然是活跃的数学研究领域；
两个问题之间的联系仍然没有完全理解。也就是说，两者之间有许多相似之处：

- 两者都被认为在经典计算机上难以计算，但都没有对此的证明。
- 两者都可以使用 Shor 算法在量子计算机上有效计算。
- 一个领域的数学进步通常很快转化为另一个领域的数学进步。
...
给定整数 :math:`a` 和正整数 :math:`b`，其中
gcd\ :math:`(a, b) = 1`，:math:`a \pmod{b}` 的*乘法阶*是满足
:math:`a^k = 1 \pmod{b}` 的最小正整数 :math:`k`。
...
像模算术一样，椭圆曲线算术用于许多公钥密码系统。
许多传统上使用模算术工作的密码系统，如 Diffie-Hellman 和 DSA，
都有椭圆曲线对应物。
...
这被称为"短 Weierstrass 形式"，是谈论椭圆曲线时最常见的形式。
还有其他几种形式主要在密码学中有应用，值得注意的是 Edwards 形式：
